...
Run `scripts/setup.sh` to verify all dependencies. Requires: Python 3.10+, Manim Community Edition v0.20+ (`pip install manim`), and ffmpeg.

> ⚠️ **MathTex / Tex 미지원**: 이 런타임에서는 `MathTex` / `Tex` 클래스(수식 렌더링)를 사용할 수 없습니다. 수식이 필요한 경우 `Text` + 유니코드 수학 기호로 대체하세요. 일반 텍스트 기반 애니메이션(`Text` with monospace fonts)은 정상 작동합니다.
...
| Text | `Text` with monospace fonts | Titles, labels, annotations |
| Video I/O | ffmpeg | Scene stitching, format conversion, audio muxing |
| TTS | ElevenLabs / Qwen3-TTS (optional) | Narration voiceover |

> ⚠️ `MathTex` / `Tex` (equation rendering) is unavailable in this runtime. Use Unicode math symbols in `Text` objects instead.
...
```python
MONO = "DejaVu Sans Mono"  # cross-platform monospace; define once at top of file

Text("Fourier Series", font_size=48, font=MONO, weight=BOLD)  # titles
Text("n=1: sin(x)", font_size=20, font=MONO)                  # labels
Text("∇L", font_size=28, font=MONO)        # use Unicode math symbols
```
...
| `references/equations.md` | Unicode math in Text, equation pacing, derivation patterns |
...
| `references/troubleshooting.md` | Font errors, animation errors, common mistakes, debugging |
