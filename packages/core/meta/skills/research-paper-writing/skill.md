...
tags: [Research, Paper Writing, Experiments, ML, AI, NeurIPS, ICML, ICLR, ACL, AAAI, COLM, Citations, Statistical Analysis]
---
...
|----------|------|-------------|
| **Overleaf/Google Docs** | Browser-based | Multiple authors editing simultaneously, no git experience |
| **Git** | `git` for version control | Technical teams, need branch-based review |
| **Shared document (Google Docs)** | Browser-based | Multiple authors editing simultaneously, real-time collaboration |
...
- [ ] Set up shared workspace (shared document platform or git repo)
...
**Notation conventions to agree on early**:
- Consistent naming for your method (use a dedicated macro or styled text)
- Citation style (author-year vs numeric)
...
```
[CITATION NEEDED: author2024]  // TODO: Verify this citation exists
```
...
**Tables**:
- Use clean table formatting — bold the best value per metric
...
- Use an appendix heading, then section labels like "A: Proofs" etc.
...
| Use spacing adjustments sparingly | 0.1-0.3 page | Low if subtle, high if obvious |
...
**Do NOT**: reduce font size, change margins, remove required sections (limitations, broader impact), or use smaller fonts for main text.
...
# Use science style (IEEE-like, clean). Include 'no-latex' for headless rendering.
...
# Always include 'no-latex' for compatibility with headless rendering.
```
...
- Use diff tools to generate a marked-up document showing changes between versions
...
```
paper/
  paper.md                    # Current working version
  paper_v1_first_draft.md     # First complete draft
  paper_v2_post_review.md     # After simulated review
  paper_v3_pre_submission.md  # Final before submission
  paper_v4_camera_ready.md    # Post-acceptance final
```
...
- [ ] No file metadata containing author names (check file properties / metadata)
...
- [ ] All tables use clean formatting (bold best values, consistent precision)
- [ ] References compile correctly (no "?" in citations)
- [ ] No text overflowing margins in critical areas
...
| **AAAI** | Strict formatting — no modifications whatsoever (use venue-approved template) |
| **COLM** | Frame contribution for language model community |

### Step 7.7: Conference Resubmission & Format Conversion

When converting between venues, **start fresh with the target venue's template**:

1. Start with a clean copy of the target venue's template (from the venue website or Overleaf)
2. Copy ONLY content sections (not formatting/preamble)
...
- [ ] Switch template to camera-ready mode (if applicable — e.g., AAAI's anon to camera config)
- [ ] Add copyright notice if required by venue
- [ ] Update any "anonymous" placeholders in text
- [ ] Verify final document renders cleanly
...
**Tools**: Presentation software (PowerPoint/Keynote, Figma, Canva.
...
- Use clearly delimited proof environments
...
|------|----------------------|
| **`terminal`** | Document compilation / formatting, git operations, launching experiments (`nohup python run.py &`), process checks |
...
| **`read_file`** / **`write_file`** / **`patch`** | Paper editing, experiment scripts, result files. Use `patch` for targeted edits to large document files. |
...
delegate_task("Draft the Methods section based on these experiment scripts and configs. 
  ...
  reproduction. Write following the target venue's formatting conventions.")
