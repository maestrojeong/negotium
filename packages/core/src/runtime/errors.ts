...
export function authRecoveryHint(agent: AgentKind): string {
  switch (agent) {
    ...
    case "maestro":
      return "Please check the DEEPSEEK_API_KEY, MOONSHOT_API_KEY, or GLM_API_KEY environment variable";
  }
}
...
export function classifyAgentError(err: unknown, agent: AgentKind): string {
  ...
  if (
    /401|authentication.error|invalid.*api.key|not logged|ANTHROPIC_API_KEY|DEEPSEEK_API_KEY|MOONSHOT_API_KEY|GLM_API_KEY/i.test(
      s,
    )
  ) {
