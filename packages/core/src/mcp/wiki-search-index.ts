...
export class WikiSearchIndex {
  ...
  /**
   * Record one document that was just written.
   *
   * The write path already holds the document text, so the index is maintained
   * incrementally and never needs to scan the content directories. `sync` stays
   * for the explicit backfill of documents that predate the write tool.
   */
  upsert(source: WikiSearchIndexSource, document: PreparedWikiSearchDocument): void {
    const id = documentId(source);
    const apply = this.#database.transaction(() => {
      this.#database.query("DELETE FROM wiki_search_fts WHERE document_id = ?").run(id);
      this.#database.query("DELETE FROM wiki_search_documents WHERE id = ?").run(id);
      this.#database
        .query(`
      ...
      this.#database
        .query("INSERT INTO wiki_search_fts(document_id, kind, tokens) VALUES (?, ?, ?)")
        .run(id, source.kind, document.tokens.join(" "));
    });
    apply.immediate();
  }
