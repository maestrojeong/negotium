...
const VAULT_DB_PATH = join(DATA_DIR, "vault.db");
mkdirSync(DATA_DIR, { recursive: true });

const vaultDb = new Database(VAULT_DB_PATH, { create: true });
...
chmodSync(VAULT_DB_PATH, 0o600);
vaultDb.exec("PRAGMA journal_mode = WAL");
vaultDb.exec("PRAGMA busy_timeout = 5000");
vaultDb.exec(`
...
{
  const cols = vaultDb.prepare("PRAGMA table_info(vault)").all() as {
    name: string;
    type: string;
  }[];
  ...
  if (uid && uid.type.toUpperCase() === "INTEGER") {
    ...
    vaultDb.exec("DROP TABLE vault");
    vaultDb.exec("ALTER TABLE vault_migrated RENAME TO vault");
    vaultDb.exec("COMMIT");
  }
}
...
function decryptRow(userId: string, key: string, storedValue: string): string {
  const decoded = decryptVaultValue(userId, key, storedValue);
  if (decoded.legacyPlaintext) {
    vaultDb
      .prepare("UPDATE vault SET value = ? WHERE user_id = ? AND key = ? AND value = ?")
      .run(encryptVaultValue(userId, key, decoded.value), userId, key, storedValue);
  }
...
export function vaultSet(userId: string, key: string, value: string, description = ""): void {
  ...
  vaultDb
    ...
    .run(userId, normalizedKey, encryptVaultValue(userId, normalizedKey, value), description);
}
...
export function vaultDeleteAllForUser(userId: string): number {
  const result = vaultDb.prepare("DELETE FROM vault WHERE user_id = ?").run(userId) as {
    changes: number;
  };
