...
const SOURCE_MCP_SERVER_FILE = fileURLToPath(new URL("./mcp-server.ts", import.meta.url));
const PACKAGED_MCP_SERVER_FILE = fileURLToPath(
  new URL("./runtime/cron/mcp-server.ts", import.meta.url),
);
...
export function createCronModule(options: CronModuleOptions = {}): NegotiumNodeModule {
  return {
    ...
    start(context) {
      ...
      const mcpServerFile =
        options.mcpServerFile ??
        (existsSync(SOURCE_MCP_SERVER_FILE) ? SOURCE_MCP_SERVER_FILE : PACKAGED_MCP_SERVER_FILE);
      ...
      const unregisterBackgroundSessions = host.registerBackgroundSessionProvider(
