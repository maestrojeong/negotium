...
| **Node module** | 호스트가 명시적으로 조합하는 선택 기능. capability와 start/stop 수명주기를 가짐 | `platform/modules.ts`, `packages/module-*` |
...
runtime-mcp-secret, vault-master-key  노드 로컬 시크릿(0600)
```

- SQLite는 WAL 모드 + busy_timeout. **한 머신에 런타임 프로세스 하나**가 규칙인 이유.
- vault는 AES 암호화, 마스터키는 상태 루트의 `vault-master-key`(0600) 또는 env.
...
- **peer/relay** (노드 간 통신): core에는 Otium의 인증·배치·relay를 넣지 않는다.
  `@negotium/adapter-otium`이 `registerNodeRequestHandler("otium-peer", ...)`로 peer HTTP 표면을
  노드 앞단에 마운트하고 RuntimeBus 이벤트를 hub로 역전송한다. direct peer 핵심은 구현됐지만
  remote ask/file/일부 hub bridge와 relay TunnelClient는 아직 adapter 후속 범위다.
  `node/topic` 발신 형태는 session-comm의 `peer-forward.ts` seam에 남아 있다.
- **cron은 선택 모듈로 구현됨**: `@negotium/module-cron`. 하나의 노드 타이머 + SQLite
  `next_run_at` 인덱스를 사용하고 job마다 pm2 프로세스를 만들지 않는다. Cron 세션은 job이 아니라
  topic이 소유한다. 같은 topic의 모든 Cron job은 직렬 실행되며 하나의 provider-neutral 대화 로그를
  공유하고, agent별 native resume ID만 그 아래에 둔다. 성공한 topic Cron 실행 5회마다 native 세션을
  회전하고 최근 대화 5턴만 새 세션으로 이어서 무한 누적을 막는다. 회전 카운터도 job별이 아니라
  topic별이다. 이 Cron 문맥은 일반 topic 세션과 분리된다.
  노드 프로세스 자체는 launchd/systemd/pm2 같은 supervisor로 재시작한다.
- **auth/멀티유저 UI**: 코어는 userId 문자열로 격리만 한다. 사용자 관리는 호스트 책임.

제품별 조합과 Otium 노드 초대/배치 불변식은 `PRODUCT-TOPOLOGY.ko.md`, 실제 peer wire 계약은
`OTIUM-COUPLING.md`를 따른다.
