...
```text
@negotium/cli
├── @negotium/node                 state directory별 장수명 로컬 node
...
Terminal TUI ── REST + cursor 기반 SSE ── 장수명 node
adapter 3개 ── @negotium/adapter-sdk의 같은 lifecycle contract
            └─ SQLite의 topic/task/runtime event/lease를 프로세스 간 공유
```
...
같은 state를 쓰더라도 channel은 별도 프로세스로 실행한다.

```bash
negotium start terminal  # 여러 개 실행 가능
negotium start telegram  # 별도 shell, 하나만 실행
negotium start otium     # 별도 shell, 하나만 실행
```

Terminal은 예외적으로 UI와 node lifecycle을 분리한다. 첫 `negotium terminal`이 인증된 loopback node를
자동 시작하고, 이후 Terminal은 모두 REST/SSE client로 같은 node에 연결한다. TUI가 정상 종료되거나
crash해도 node와 진행 중 agent turn은 계속 산다.

```bash
negotium status
negotium stop
negotium serve --port=7777          # foreground node
negotium terminal --embedded        # 복구/개발용 in-process 모드
negotium terminal --connect=http://127.0.0.1:7777
```

Node control API는 `127.0.0.1`에만 bind하고 state directory의 mode-0600 bearer token으로 인증한다.
state directory별 SQLite singleton lease와 endpoint metadata가 중복 시작을 막는다. `--port=0`은 자동 시작
때 사용 가능한 loopback port를 고른다. Telegram과 Otium은 현재 중복 polling/backflow를 막는 자체
state-directory singleton lease와 host lifecycle을 유지한다. 모든 프로세스의 topic/message, runtime event,
turn request, active-turn lease는 SQLite로 공유된다.
...
| Cron schedule | SQLite + singleton Cron module | 공유 |
| Telegram chat/thread ID | Telegram mapping DB | Telegram 전용 |
| Otium host room ID/peer request | Otium binding DB | Otium 전용 |
| agent turn, Node worker/MCP | 장수명 `@negotium/node` | TUI 종료와 독립 |
| TUI selection/scroll/composer | Terminal process memory | Terminal client 전용 |
...
Terminal은 node control API로 topic 목록을 읽으므로 추가 mapping 없이 모든 보이는 topic을 선택할 수 있다.
...
Telegram 개인 DM은 별도 `tg-*` topic을 만들지 않고 Terminal과 같은 개인 `General(manager)`에 연결된다.
첫 DM과 `/start`는 영어 온보딩을 보여준다. 포럼 supergroup에서 Topics를 켜고 봇을 **Manage Topics**
권한의 관리자로 승격하면 `/connect` 없이 자동 연결된다. 봇이 승격 이벤트를 놓쳤어도 허용된 사용자가
보낸 첫 그룹 메시지에서 봇과 발신자의 관리자 권한을 확인한 뒤 복구한다. 연결된 그룹 ID와 thread-topic
mapping은 Telegram 전용 SQLite DB에 저장되므로 Telegram 프로세스를 재시작해도 유지된다.

DM General과 그룹 General이 같은 manager topic을 사용하더라도 Telegram에서 시작한 turn의 답변은 그
turn을 시작한 Telegram 위치로만 돌아간다. 따라서 DM에서 시작한 답변이 그룹에 동시에 전송되거나 그
반대가 되는 fan-out은 하지 않는다. 반면 Terminal에서 시작한 General의 assistant 답변은 Telegram origin이
없으므로 같은 General에 연결된 DM과 그룹 General 모두에 전송해 channel view를 동기화한다.

일반 topic의 `/new` session reset은 보이는 message history를 보존하지만 reset 전에 접수된 active/queued
turn을 모두 취소한다. reset 뒤에 접수된 요청은 새 provider context에서 시작한다. 개인 General manager는
reset하지 않는다.
