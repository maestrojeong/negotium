...
Set `DEEPSEEK_API_KEY` for DeepSeek models, `MOONSHOT_API_KEY` for Kimi models, or `GLM_API_KEY`
for GLM models in the working directory's ignored `.env` to authenticate Maestro Agent SDK.
`MOONSHOT_BASE_URL` and `GLM_BASE_URL` optionally override the corresponding provider endpoint.
`GEMINI_API_KEY` is optional for image QA.
